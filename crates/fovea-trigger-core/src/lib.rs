//! fovea-trigger-core
//!
//! Stable types and arithmetic primitives for the fovea-trigger subproject.
//!
//! Scope (deliberate):
...
//!
//! Out of scope:
//! - H.264 bitstream parsing — lives in `fovea-trigger-stream` behind `ffmpeg-next`.
//! - Pixel decoding — `fovea-trigger-stream` exposes a separate path.
...
///
/// `mvs` is owned. Producers ([`fovea-trigger-stream`]) should reuse the buffer
