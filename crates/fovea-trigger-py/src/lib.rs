...
use fovea_trigger_core::triggers::{
...
use fovea_trigger_core::Trigger as RsTrigger;
use fovea_trigger_stream::{FfmpegSource, NetworkOptions, OpenOptions, RtspTransport, SourceError};

/// Convert a fovea-trigger-stream error into the most idiomatic Python exception.
...
impl TriggerImpl {
    fn evaluate(&mut self, packet: &fovea_trigger_core::MvPacket) -> Option<fovea_trigger_core::Event> {
        match self {
...
///
/// Wraps `FusionTrigger` from `fovea-trigger-core`. Combines motion energy,
        ...
        loop {
            ...
            let (seq, fired) = {
                ...
                *seq_lock += 1;
                ...
                let mut fired_event: Option<fovea_trigger_core::Event> = None;
...
#[pymodule]
fn _fovea_trigger(m: &Bound<'_, PyModule>) -> PyResult<()> {
