# 2026-04-29 — pixel-diff baseline vs fovea-trigger density (M3 8-core, cctv-sample)
...
On Apple M3 (8-core) parsing the same 30 fps 1080p H.264 clip, fovea-trigger
...
fovea-trigger has ~30% CPU headroom while pixel-diff is at 45%.
...
1080×1920, H.264 30 fps clip used in the fovea-trigger density smoke run.
...
measure is the decode + diff loop, equivalent to fovea-trigger's "high
motion-threshold + long interval" parse-path baseline.

The fovea-trigger numbers are reproduced verbatim from
...
|    1 | fovea-trigger   |  99.5 | **14.9** |  6.71 |  87.5 |
|    2 | pixel-diff | 329.0 | **36.0** |  9.13 | 198.9 |
|    2 | fovea-trigger   |  95.1 | **17.0** |  5.60 |  88.0 |
|    4 | pixel-diff | 153.8 | **37.9** |  4.06 | 177.3 |
|    4 | fovea-trigger   |  90.1 | **20.9** |  4.30 |  88.7 |
|    8 | pixel-diff |  87.3 | **43.0** |  2.03 | 178.8 |
|    8 | fovea-trigger   |  66.1 | **26.5** |  2.50 |  86.4 |
|   16 | pixel-diff |  45.4 | **44.9** |  1.01 | 140.8 |
|   16 | fovea-trigger   |  29.9 | **31.3** |  1.00 |  70.1 |
...
pixel-diff worker uses ~4–5 cores at N=1) while fovea-trigger's parse path
is single-threaded.

## Findings

1. **Per-realtime-stream CPU cost: fovea-trigger ~2× cheaper across the
   board.** At N=1 it is 14.9% vs 30.1%; the gap stays around 1.4–2×
   through N=16.
2. **RSS: fovea-trigger ~2× smaller per worker.** 70–88 MB vs 140–200 MB.
...
3. **Realtime ceiling on this box and clip: ~16 for both.** Throughput
   ...
   gap.
4. **Headroom at the ceiling.** At N=16 each fovea-trigger worker uses
   31.3% of a realtime stream's CPU budget, vs 44.9% for pixel-diff.
   On a wider box or a longer clip, fovea-trigger's lower per-stream cost
...
- **`--threshold 1e15` means zero fires for pixel-diff.** Latency and
  ...
  matching the fovea-trigger smoke setup. Trigger-quality numbers (FP rate,
...
# Compare against:
make density-bench   # fovea-trigger equivalent, see 2026-04-28-density-smoke.md
```

Raw per-stage JSON:
- `benchmarks/_runs/pixeldiff-smoke/summary.json` (gitignored under `_runs/`)
- `benchmarks/_runs/density-smoke/summary.json` (same, fovea-trigger side)
...
- Sweep `--threshold` against the same clip with normal traffic motion
  ...
  fovea-trigger MotionTrigger over the same source for an apples-to-apples
