...
public:
  ...
  */
  ...
  // --- de265_internals: prediction-block MV exposure (added by fovea-mv) ---
  //
  // Read-only access to the already-populated `pb_info` MetaDataArray and
  // the per-slice reference-picture-POC lists. The MV data itself is
  // populated by `set_mv_info(...)` during normal HEVC inter prediction,
  // so these accessors do not change the decoder's behavior — only its
  // public surface.

  /// PB grid layout. `log2_unit_size` is typically 2 (4-pixel cells).
  void internals_get_PB_layout(int *width_in_units,
                               int *height_in_units,
                               int *log2_unit_size) const;

  /// Fill `out` with one entry per PB cell (raster order, length =
  /// `width_in_units * height_in_units`). See `de265_internals.h` for
  /// the entry shape.
